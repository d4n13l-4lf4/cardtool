...
def test_gen_json_card(config_file, expected_file, tmp_path, data_resolver: Callable[[str], str]):
    runner = CliRunner()
    with runner.isolated_filesystem(temp_dir=tmp_path):
        ...
        result = runner.invoke(gen_card, [
            '--config', config_file_fullpath, '--format', 'json', '--name', outfile_fullpath
        ])
